...
func Test_transx_new(t *testing.T) {
	tests := []struct {
		name    string
		mB      int
		want    int
	...
	for _, tt := range tests {
		var transx transpStruct
		t.Run(tt.name, func(t *testing.T) {
			err := transx.new(tt.mB)
			if got := len(transx.tab); got != tt.want {
...
func Test_flipSide(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := flipSidex(tt.key); got != tt.want {
...
func Test_initx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 12*64; i++ {
				for j := i + 1; j < 12*64; j++ {
					if randPcSqx[i] == randPcSqx[j] {
						t.Errorf("index %v and %v has the same value %v", i, j, randPcSqx[i])
					}
				}
				for j := 0; j < 8; j++ {
					if randPcSqx[i] == randEpx[j] {
						t.Errorf("randPcSq[%v] is equal to randEp[%v]: %v ", i, j, randPcSqx[i])
					}
				}
				for j := 0; j < 4; j++ {
					if randPcSqx[i] == randCastlx[j] {
						t.Errorf("randPcSq[%v] is equal to randCastl[%v]: %v ", i, j, randPcSqx[i])
					}
				}
			}
		})
...
func Test_Trans(t *testing.T) {
	...
	for _, e := range tests {
		...
		idx := key & uint64(transx.mask)
		t.Log("idx", idx)
		transx.store(key, mv, depth, ply, sc, scoreType)
	}
	...
	for ix, e := range tests {
		...
		ok := false
		if transMove, transScore, transScType, ok = transx.retrieve(key, depth, ply); ok {
			// retrieve found ok
			switch ix {
			case 2: // special case
...
func Test_transpStruct_retrieve(t *testing.T) {
	...
	tests := []struct {
		...
		{"not in tab", transx, args{0x0101010101010101, 4, 4}, noMove, noScore, 0, false},
		{"in tab", transx, args{0x0404040404040404, 4, 1}, 40, 4, 0, true},
		{"lower depth", transx, args{0x0404040404040404, 1, 1}, 40, 4, 0, true},
		{"higher depth", transx, args{0x0404040404040404, 6, 1}, 40, 4, 0, false},
	}
	// make some stores
	transx.store(0x0fffffffffffffff, 10, 1, 1, 1, 0)
	transx.store(0x0202020202020202, 20, 2, 2, 2, 0)
	transx.store(0x0303030303030303, 30, 3, 1, 3, 0)
	transx.store(0x0404040404040404, 40, 4, 3, 4, 0)
	transx.store(0x0606060606060606, 60, 6, 4, 5, 0)
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
